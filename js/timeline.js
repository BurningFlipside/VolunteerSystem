...
function processDepartments(depts) {
  ...
  for(let dept of depts) {
    console.log(dept);
    if(!dept.available && !dept.isAdmin) {
      continue;
    }
...
function addNonStandardViewAlert() {
  ...
  addNotification(content, 'You are seeing a non-default view of the volunteer system from the last time you viewed the site. To restore the default view click <a href="#" id="nonStandardView" class="alert-link" onclick="clearView();">here.</a>', 'alert-primary');
}
...
function initPage() {
  ...
  if(myView === null && localStorage !== null) {
